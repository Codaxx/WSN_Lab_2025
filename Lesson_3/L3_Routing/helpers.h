...
// turns an LED on or off and keeps track of the number of packets
// with specific LED ID, so LEDS will not be tured off if there are more
// packets of this ID present.
void turn_on(uint8_t led_id);
void turn_off(uint8_t led_id);

void print_lookup_table(l_table table, uint8_t number_of_nodes);

// defines the led ID based on the node id.
